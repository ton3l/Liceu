...
function TBody({ list, edit, setEdit }) {
    return (
        <TableBody> 
            {
                list.map((item, index) => (
...
function Table({ list }) {
    const [edit, setEdit] = useState(Array(list.length).fill(true));

    // Se o tamanho da lista mudar, sincronize o estado:
    useEffect(() => {
        setEdit(Array(list.length).fill(true));
    }, [list.length]);
    ...
    return (
        <TableContainer component={Paper} elevation={3} className={styles.tableContainer}>
            <MaterialTable>
                <THead />
                <TBody list={list} edit={edit} setEdit={setEdit} />
            </MaterialTable>
        </TableContainer>
    );
}
